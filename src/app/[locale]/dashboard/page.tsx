...
import { Plus, ExternalLink, Trash2, Edit3, UserPlus, CalendarPlus, ListChecks, MessageCircle, FileText } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  const updateDashboardMutation = useMutation({
    ...
    onError: (error) => {
      ...
      toast({
        title: 'Eroare',
        description: error instanceof Error ? error.message : 'Salvarea modificărilor a eșuat. Încercați din nou.',
        variant: 'destructive',
      })
    },
  })
  ...
  useEffect(() => {
    if (patientsError) {
      toast({
        title: 'Eroare la încărcarea pacienților',
        description: patientsError instanceof Error ? patientsError.message : 'Încărcarea datelor pacienților a eșuat',
        variant: 'destructive',
      })
    }
  }, [patientsError, toast])
  ...
  useEffect(() => {
    if (tasksError) {
      toast({
        title: 'Eroare la încărcarea sarcinilor',
        description: tasksError instanceof Error ? tasksError.message : 'Încărcarea datelor sarcinilor a eșuat',
        variant: 'destructive',
      })
    }
  }, [tasksError, toast])
  ...
  useEffect(() => {
    if (chatError) {
      toast({
        title: 'Eroare la încărcarea chatului',
        description: chatError instanceof Error ? chatError.message : 'Încărcarea datelor chatului a eșuat',
        variant: 'destructive',
      })
    }
  }, [chatError, toast])
  ...
  useEffect(() => {
    if (appointmentsError) {
      toast({
        title: 'Eroare la încărcarea programărilor',
        description: appointmentsError instanceof Error ? appointmentsError.message : 'Încărcarea datelor programărilor a eșuat',
        variant: 'destructive',
      })
    }
  ...
  const stats: {
  ...
  }[] = [
    {
      ...
      icon: '👥',
      action: { href: '/dashboard/patients/new', label: 'Adaugă pacient nou', Icon: UserPlus },
    },
    {
      name: 'Programări astăzi',
      value: (todayAppointments?.length || 0).toString(),
      icon: '📅',
      action: { href: '/dashboard/appointments/new', label: 'Programează consultație', Icon: CalendarPlus },
    },
    {
      name: 'Sarcini în așteptare',
      value: (tasks?.length || 0).toString(),
      icon: '✅',
      action: { href: '/dashboard/tasks', label: 'Vezi sarcinile', Icon: ListChecks },
    },
    {
      name: 'Mesaje necitite',
      value: (unreadMessagesData?.unreadCount || 0).toString(),
      icon: '💬',
      action: { href: '/dashboard/chat', label: 'Deschide chatul', Icon: MessageCircle },
    },
  ]
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-gray-900">Panou principal</h1>
        <p className="mt-1 text-sm text-gray-500">
          Bine ați revenit! Iată o privire de ansamblu asupra clinicii.
        </p>
        <div className="mt-4">
          <Button asChild>
              ...
              <div className="p-5">
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" asChild>
                    <EmbeddedLink href={stat.action.href} aria-label={stat.action.label} title={stat.action.label}>
        ...
        >
          <Card className="h-[240px] flex flex-col">
            <CardHeader className="flex flex-row items-center space-y-0 pb-3 flex-shrink-0">
              ...
              <CardTitle className="text-lg">Notă rapidă</CardTitle>
            </CardHeader>
            <CardContent className="flex flex-col flex-1 min-h-0">
              {isDashboardLoading ? (
                <div className="flex items-center justify-center h-full">
                  <div className="text-sm text-gray-500">Se încarcă notițele...</div>
                </div>
              ) : (
                <>
                  <Textarea
                    placeholder="Notițe rapide, memento-uri sau idei..."
                    value={quickNote}
                  ...
                  <p className="text-xs text-gray-500 mt-2 flex-shrink-0 flex items-center">
                    <span>Se salvează automat în contul personal. Vizibil doar pentru dvs.</span>
                    {updateDashboardMutation.isPending && (
                      <span className="ml-2 text-red-500">Se salvează...</span>
                    )}
                  </p>
                </>
              )}
        ...
        >
            ...
            <>
              ...
              <div className="absolute top-[-200px] left-[-10px] right-0 z-50 p-4 bg-gradient-to-b from-gray-300 to-blue-100 border rounded-lg shadow-lg">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-medium text-gray-900">Adaugă link nou</h3>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => {
          ...
          <Card className="h-[240px] flex flex-col">
            <CardHeader className="flex flex-row items-center space-y-0 pb-3 flex-shrink-0">
              ...
              <CardTitle className="text-lg">Linkuri rapide</CardTitle>
            </CardHeader>
            <CardContent className="flex flex-col flex-1 min-h-0">
              {isDashboardLoading ? (
                <div className="flex items-center justify-center h-full">
                  <div className="text-sm text-gray-500">Se încarcă linkurile...</div>
                </div>
              ) : (
                <>
