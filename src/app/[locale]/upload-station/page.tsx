...
function photoTypeLabel(category: ImageCategory, count?: number): string {
  const base = category === 'BEFORE_PHOTO' ? 'Înainte' : 'După';
  if (count === undefined) return base;
  const noun = count === 1 ? 'fotografie' : 'fotografii';
  return `${base} (${noun})`;
}
...
export default function UploadStationPage() {
  ...
  const handleUnlock = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPin('');
      await checkAuth();
      toast.success('Stația de încărcare a fost deblocată');
    } catch {
      toast.error('Deblocarea a eșuat');
    } finally {
  ...
  const uploadFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || apiErrors.failedToUpload);

      const label = category === 'BEFORE_PHOTO' ? 'Înainte' : 'După';
      const noun = data.count === 1 ? 'fotografie' : 'fotografii';
      toast.success(`${data.count} ${noun} ${label.toLowerCase()} adăugate pentru ${data.patient}`);
      ...
      toast.error(err instanceof Error ? err.message : apiErrors.failedToUpload);
    } finally {
      ...
      <div className="flex min-h-screen flex-col items-center justify-center p-6">
        <div className="w-full max-w-sm space-y-6">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Introduceți PIN-ul clinicii pentru a încărca fotografii</p>
          </div>
          <form onSubmit={handleUnlock} className="space-y-4">
            ...
            <Button type="submit" className="w-full h-12 text-lg" disabled={isUnlocking || !pin}>
              {isUnlocking ? 'Se deblochează...' : 'Deblochează'}
            </Button>
          </form>
        </div>
      </div>
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-lg flex-col">
      ...
      <main className="flex-1 space-y-6 p-4">
          ...
          {selectedPatient ? (
            <div className="flex items-center justify-between rounded-lg border bg-white p-3">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {selectedPatient.patientCode || 'Fără cod'} · {new Date(selectedPatient.dateOfBirth).toLocaleDateString('ro-RO')}
                </p>
              </div>
            ...
            <div className="space-y-2">
              <div className="relative">
                ...
                <Input
                  placeholder="Căutați nume, cod sau dată nașterii..."
                  value={searchQuery}
              ...
              <div className="max-h-48 overflow-y-auto rounded-lg border bg-white">
                {filteredPatients.length === 0 ? (
                  <p className="p-4 text-center text-sm text-muted-foreground">Niciun pacient găsit</p>
                ) : (
                  filteredPatients.map((patient) => (
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground">
                        {patient.patientCode || 'Fără cod'} · {new Date(patient.dateOfBirth).toLocaleDateString('ro-RO')}
                      </p>
                    </button>
                  ))
                )}
        ...
        {/* Before / After toggle */}
        <section className="space-y-3">
          <Label className="text-sm font-medium">Tip fotografie</Label>
        ...
        <section className="space-y-3">
          ...
          >
            ...
            {isUploading ? 'Se încarcă...' : 'Fă o fotografie'}
          </Button>
          <Button
            variant="outline"
        ...
        {recentUploads.length > 0 && (
          <section className="space-y-2">
            <Label className="text-sm font-medium">Încărcări recente</Label>
            <div className="space-y-2">
                ...
                <div key={upload.id} className="flex items-center gap-3 rounded-lg border bg-white p-3 text-sm">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-muted-foreground">
                      {upload.count} {photoTypeLabel(upload.type)} · {upload.at}
                    </p>
                  </div>
                </div>
              ))}
