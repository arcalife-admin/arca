...
export default function OrdersPage() {
  ...
  const handleApproveRequest = async (requestId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        appAlert(error.error || 'Failed to approve request', { title: 'Error' });
      }
    } catch (error) {
      console.error('Error approving request:', error);
      appAlert('Failed to approve request', { title: 'Error' });
    }
  };
  ...
  const handleRejectRequest = async (requestId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        appAlert(error.error || 'Failed to reject request', { title: 'Error' });
      }
    } catch (error) {
      console.error('Error rejecting request:', error);
      appAlert('Failed to reject request', { title: 'Error' });
    }
  };
  ...
  const handleQuickOrderSubmit = async () => {
    ...
    if (response.ok) {
      ...
      const error = await response.json();
      appAlert(error.error || 'Failed to create order', { title: 'Error' });
    }
  };
  ...
  const handleUpdateRepairStatus = async (repairId: string, status: string) => {
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        appAlert(error.error || 'Failed to update repair status', { title: 'Error' });
      }
    } catch (error) {
      console.error('Error updating repair status:', error);
      appAlert('Failed to update repair status', { title: 'Error' });
    }
  };
