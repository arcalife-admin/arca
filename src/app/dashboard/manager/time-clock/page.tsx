...
export default function ManagerTimeClockPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: 'Could not load time clock',
  ...
  useEffect(() => {
    ...
    if (!canAccess) {
      toast({
        title: 'Access denied',
        description: 'Only managers and organization owners can open this page.',
        variant: 'destructive',
      })
  ...
  const postEvent = async (eventType: 'CLOCK_IN' | 'CLOCK_OUT') => {
    if (!selectedUserId) {
      toast({
        title: 'Select a user',
        description: 'Choose who is clocking in or out.',
        variant: 'destructive',
      })
      return
    }
  ...
  const saveEventDialog = async () => {
    ...
    try {
      ...
      toast({
        title: editingId !== null ? 'Event updated' : 'Event added',
        description: 'Time clock data was saved.',
      })
  ...
  const confirmDeleteEvent = async () => {
    ...
    try {
      ...
      toast({ title: 'Event deleted', description: 'The punch was removed.' })
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto px-4 py-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Manual clock-in/out for testing until NFC scanners are connected. Add, edit, or delete
            punches when someone forgets or the time was wrong. Hours use the server calendar for
            today, this week (Mon–Sun), and this month.
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Test: clock in or out</CardTitle>
          <CardDescription>
            Select a team member, then record clock in or clock out as if they tapped the NFC
            reader.
          </CardDescription>
        </CardHeader>
                ...
                {users
                  ...
                  .map((u) => (
                    <SelectItem key={u.id} value={u.id} className="text-gray-400">
                      {u.firstName} {u.lastName} (disabled)
                    </SelectItem>
                  ))}
              </SelectContent>
            </Select>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Worked time from paired clock-in / clock-out events (same window as below).
          </CardDescription>
        </CardHeader>
                ...
                return (
                  <TableRow key={u.id}>
                    <TableCell className="font-medium">
                      {h.displayName}
                      {u.isDisabled ? (
                        <span className="text-gray-400 text-xs ml-2">(disabled)</span>
                      ) : null}
                    </TableCell>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <CardTitle>Schedule / event log</CardTitle>
              <CardDescription>
                Recent events (loaded for the last 30 days). Filter by calendar period. Use Add
                event or row actions to fix mistakes.
              </CardDescription>
            </div>
            <Button
              type="button"
                ...
                <TabsContent key={tab} value={tab} className="mt-4">
                  {ranges && (
                    <p className="text-xs text-gray-500 mb-3">
                      {tab === 'day' && (
                        <>
                          {format(new Date(ranges.day.start), 'PPP')} —{' '}
                          {format(new Date(ranges.day.end), 'p')}
                        </>
                      )}
                      {tab === 'week' && (
                        <>
                          Week of {format(new Date(ranges.week.start), 'PPP')} —{' '}
                          {format(new Date(ranges.week.end), 'PPP p')}
                        </>
                      )}
                      {tab === 'month' && <>{format(new Date(ranges.month.start), 'MMMM yyyy')}</>}
                    </p>
                  )}
                  <Table>
                    <TableHeader>
                        ...
                        rows.map((ev) => (
                          <TableRow key={ev.id}>
                            <TableCell className="whitespace-nowrap tabular-nums">
                              {format(new Date(ev.occurredAt), 'PPp')}
                            </TableCell>
                            <TableCell>
                            ...
                            <TableCell>
                              {ev.eventType === 'CLOCK_IN' ? (
                                <Badge className="bg-emerald-600">Clock in</Badge>
                              ) : (
                                <Badge variant="secondary">Clock out</Badge>
                              )}
                            </TableCell>
      ...
      <Dialog open={eventDialogOpen} onOpenChange={setEventDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editingId !== null ? 'Edit punch' : 'Add punch'}</DialogTitle>
            <DialogDescription>
              Set who, clock in or out, and the exact date and time. This updates hours for today,
              week, and month automatically.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label htmlFor="tc-user">User</Label>
            ...
            <div className="space-y-2">
              <Label htmlFor="tc-when">Date &amp; time</Label>
              <Input
                id="tc-when"
                type="datetime-local"
      ...
      <AlertDialog open={deleteTarget !== null} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This removes the clock-in or clock-out event. Hours for that day may change. This
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
